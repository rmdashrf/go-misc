...
// Sets the entries of the cookiejar to the supplied entries. Callers should
// not use the supplied map after this call.
func (j *Jar) SetEntries(new map[string]map[string]Entry) {
	j.mu.Lock()
	j.entries = new
	j.mu.Unlock()
}
